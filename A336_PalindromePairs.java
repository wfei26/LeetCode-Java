...
public class A336_PalindromePairs {
    public static void main(String[] args) {
        A336_PalindromePairs solution = new A336_PalindromePairs();
    }
        ...
        for (int i = 0; i < words.length; i++) {
            for (int j = 0; j <= words[i].length(); j++) {
                ...
                if (isPalindrome(str1)) {
                    ...
                    // WARNING: MUST CHECK whether str.length() is equal to 0 in either if statement, because we need
                    // to make sure we do not add duplicate pairs (if str1.length() == 0, both of str1 and str2 will
                    // from input array)
